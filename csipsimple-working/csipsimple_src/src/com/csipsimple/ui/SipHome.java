    ...
    public void addTestAccount() {
        ...
        if (c != null) {
        	try {
        		int index = 0;
        		if(c.getCount() > 0) {
        			Log.d(THIS_FILE, "@ACC iterating...");
        			c.moveToFirst();
        			do {
        				SipProfile account = new SipProfile(c);
        				if (account.display_name.equals(accname)) {
        					return;
        				}
        				index ++;
...
//    	String ip = "137.226.58.37";
    	String ip = "128.59.21.232";
    	String fullUser = "gandalf@"+ip;
