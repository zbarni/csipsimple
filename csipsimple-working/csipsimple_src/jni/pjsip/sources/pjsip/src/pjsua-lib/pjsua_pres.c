...
static pj_str_t EVERY_COMPONENT = { "EVERYCOMPONENT", 14 };
...
//@zajzi @zf
/**
 * If component_id is EVERYCOMPONENT, this function removes all component
 * subscriptions for the buddy.
 */
void pjsua_remove_component_subscription(pj_str_t component_id,pjsua_buddy_id buddy_id)
{
	pjsua_buddy *buddy;
	pjsip_tx_data *tdata;
	pj_status_t status;
	...
	if (pj_strcmp(&EVERY_COMPONENT,&component_id) == 0) {
		remove_single = -1;
	}

	// find component subscription in buddy array
	for (index=0; index<(int)PJ_ARRAY_SIZE(buddy->comp_subscriptions); ++index) {
		if (buddy->comp_subscriptions[index].comp_name.slen == 0) {
			continue;
		}
		...
		c_sub = &(buddy->comp_subscriptions[index]);
		if (pj_strcmp(&component_id,&c_sub->comp_name) == 0 || remove_single == -1) {
			if (c_sub->sub == NULL)
				continue;

			if (pjsip_evsub_get_state(c_sub->sub) == PJSIP_EVSUB_STATE_TERMINATED) {
				c_sub->sub = NULL;
				continue;
			}

			PJ_LOG(5,(THIS_FILE, "@zbuddy Buddy %d: unsubscribing component [%.*s]", buddy_id,
					component_id.slen, component_id.ptr));
			pj_log_push_indent();

			status = pjsip_pres_initiate( c_sub->sub, 0, &tdata);
			if (status == PJ_SUCCESS) {
				pjsua_process_msg_data(tdata, NULL);
				status = pjsip_pres_send_request( c_sub->sub, tdata );
			}

			if (status != PJ_SUCCESS && c_sub->sub) {
				pjsip_pres_terminate(c_sub->sub, PJ_FALSE);
				c_sub->sub = NULL;
				pjsua_perror(THIS_FILE, "@zbuddy Unable to unsubscribe presence from component",
						status);
			}

			pj_log_pop_indent();

			// we only had to delete one, so return
			if (remove_single == 1) {
				return;
			}
		}
...
PJ_DEF(pj_status_t) pjsua_buddy_del(pjsua_buddy_id buddy_id)
{
	...
	//@zajzi
	pjsua_remove_component_subscription(EVERY_COMPONENT,buddy_id);
...
{
	PJ_LOG(4,(THIS_FILE, "@zbuddy server subscription has changed"));
...
//		PJ_LOG(4,(THIS_FILE, "@zbuddy From uri is [%.*s] \n remote info str is [%.*s]"
//				""
//				"",
//				acc_id));
...
{
	PJ_LOG(4,(THIS_FILE, "@zbuddy buddy timer callback, will update presence, damn"));
...
{
	PJ_LOG(4,(THIS_FILE, "@zbuddy buddy resubrscribe"));
    if (buddy->timer.id) {
...
{
	PJ_LOG(4,(THIS_FILE, "@zbuddy buddy on state"));
    pjsua_buddy *buddy;
...
//@zajzi
/* Callback called when *client* subscription state has changed. */
static void pjsua_evsub_on_component_state(pjsip_evsub *sub, pjsip_event *event)
{
	PJ_LOG(4,(THIS_FILE, "@zbuddy buddy on component state"));
	pjsua_buddy *buddy;
	pjsua_buddy_component_sub *c_sub;
	pj_str_t *component;
	int index;
	component = pjsip_evsub_get_mod_data(sub, 21);

	PJ_UNUSED_ARG(event);

	pj_pool_t *tmp_pool =  pjsua_pool_create("tmpcomponen", 512, 256);
	...
	if (buddy) {
		...
		for (index=0; index<(int)PJ_ARRAY_SIZE(buddy->comp_subscriptions); ++index) {
			...
			c_sub = &(buddy->comp_subscriptions[index]);
			if (pj_strcmp(component,&c_sub->comp_name) == 0) {
				break;
			}
		}

		PJ_LOG(4,(THIS_FILE,
				"@zbuddy Presence subscription to %.*s is %s",
				(int)pjsua_var.buddy[buddy->index].uri.slen,
				pjsua_var.buddy[buddy->index].uri.ptr,
				pjsip_evsub_get_state_name(sub)));
		pj_log_push_indent();
		...
		if (pjsip_evsub_get_state(sub) == PJSIP_EVSUB_STATE_TERMINATED) {
			...
			PJ_LOG(4,(THIS_FILE,"@zbuddy State is terminated"));
			//
			//
			//			int resub_delay = -1;
			//
		...
		// get component name
		// TODO WARNING, 21 is hack, there are 18 modules registered..

		pj_str_t tmp;
		pj_str_t *sub_state;
		const char *seq = pjsip_evsub_get_state_name(sub);

		//		pj_strdup3(tmp_pool,seq);
		tmp = pj_str(seq);

		// notify application
		if (pjsua_var.ua_cfg.cb.on_buddy_component_state)
			(*pjsua_var.ua_cfg.cb.on_buddy_component_state)(buddy->index, *component, tmp);

		PJ_LOG(4,(THIS_FILE,"@zbuddy State change component [%.*s] state [%.*s]",
				component->slen, component->ptr, tmp.slen,tmp.ptr));
		...
		if (pjsip_evsub_get_state(sub) == PJSIP_EVSUB_STATE_TERMINATED) {
			...
			buddy->status.info_cnt = 0;
			buddy->dlg = NULL;
			pjsip_evsub_set_mod_data(sub, 21, NULL);
			pjsip_evsub_set_mod_data(sub, pjsua_var.mod.id, NULL);
		}
				     ...
				     pjsip_event *event)
{
	...
	PJ_LOG(4,(THIS_FILE, "@zbuddy Transaction state has changed"));
    /* Note: #937: no need to acuire PJSUA_LOCK here. Since the buddy has
     *   a dialog attached to it, lock_buddy() will use the dialog
     *   lock, which we are currently holding!
     */
...
{
    pjsua_buddy *buddy;
	pj_str_t *str = pjsip_evsub_get_mod_data(sub, 21);

	PJ_LOG(4,(THIS_FILE, "@zbuddy Notify received"));
	if (str != NULL)
		PJ_LOG(4,(THIS_FILE, "@zbuddy Notify received and it's for component : %.*s",
				str->slen, str->ptr));
    /* Note: #937: no need to acuire PJSUA_LOCK here. Since the buddy has
     *   a dialog attached to it, lock_buddy() will use the dialog
     *   lock, which we are currently holding!
...
static void subscribe_to_component (pjsua_buddy_id buddy_id, int sub_index) {
	...
	pres_callback.on_evsub_state = &pjsua_evsub_on_component_state;
	...
	pj_strdup_with_null(tmp_pool,&param_uri,&buddy->uri);
	pj_strcat2(&param_uri,";component=");
	pj_strcat(&param_uri,&(c_sub->comp_name));

	PJ_LOG(4,(THIS_FILE, "@zbuddy %d: subscribing to component %.*s, ,using account %d ; buddy uri : %.*s",
			buddy_id,c_sub->comp_name.slen, c_sub->comp_name.ptr, acc_id, buddy->uri.slen, buddy->uri.ptr));
	pj_log_push_indent();
	...
	pjsip_evsub_set_mod_data(c_sub->sub, 21, &c_sub->comp_name);
...
static void subscribe_buddy_presence(pjsua_buddy_id buddy_id)
{
	PJ_LOG(4,(THIS_FILE, "@zbuddy Subscribe to buddy presence"));
    pjsip_evsub_user pres_callback;
...
{
	PJ_LOG(4,(THIS_FILE, "@zbuddy Refresh client subscription"));
    unsigned i;
    pj_status_t status;
...
{
	PJ_LOG(4,(THIS_FILE, "@zbuddy prese timer callback"));
    unsigned i;
